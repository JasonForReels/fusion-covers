...
import { siAppletv, siMax, siHbomax, siHbo, siParamountplus, siCrunchyroll, siYoutube, siYoutubetv, siPlex, siJellyfin, siStarz, siShowtime, siMubi, siTubi, siFubo, siRoku, siTwitch, siItvx, siChannel4, siSky, siViaplay, siDazn, siApple, siTrakt, siImdb, siThemoviedatabase, siLetterboxd, siAnilist, siMyanimelist, siKodi, siStremio, siTvtime } from 'simple-icons'
...
export const LOGOS = [siAppletv, siMax, siHbomax, siHbo, siParamountplus, siCrunchyroll, siYoutube, siYoutubetv, siPlex, siJellyfin, siStarz, siShowtime, siMubi, siTubi, siFubo, siRoku, siTwitch, siItvx, siChannel4, siSky, siViaplay, siDazn, siApple, siTrakt, siImdb, siThemoviedatabase, siLetterboxd, siAnilist, siMyanimelist, siKodi, siStremio, siTvtime].map((i) => ({
  id: i.slug,
  title: i.title,
  hex: '#' + i.hex,
  path: i.path,
...
// Netflix wordmark extracted from netflix.com's brand SVG (images.ctfassets.net), bundled locally.
const IMAGE_LOGOS = [
  { id: 'netflix', title: 'Netflix', hex: '#E50914', src: netflixLogo, defaultTint: 'original' },
...
// Bundled wordmark matching a TMDB brand (provider/network), used by "Use as logo" instead of TMDB's icon.
const norm = (s) => String(s ?? '').toLowerCase().replace(/[^a-z0-9]/g, '')
export const imageLogoForBrand = (b) =>
  b && IMAGE_LOGOS.find((l) => {
    const h = BRAND_HINTS[l.id] ?? {}
    return (b.kind === 'network' && b.id === h.network) || (b.kind === 'provider' && [h.provider].flat().map(norm).includes(norm(b.name)))
  })
